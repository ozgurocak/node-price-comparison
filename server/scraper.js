...
async function scrapeProductVatan(browser, productLink) {
    ...
    const product = {
        ...
        "brand": await page.$x('/html/body/main/div/div[1]/div/div/div/ul/li[4]/a').then((res) => page.evaluate(el => el.textContent, res[0])),
        ...
        "processor": await page.$x('//div[@id="urun-ozellikleri"]//td[text()="İşlemci Teknolojisi"]/following-sibling::td/p[1]').then((res) => page.evaluate(el => el.textContent, res[0])),
        ...
        "screen": await page.$x('//div[@id="urun-ozellikleri"]//td[text()="Ekran Boyutu"]/following-sibling::td/p[1]').then((res) => page.evaluate(el => el.textContent, res[0])),
...
async function scrapeListTrendyol(browser) {
    ...
    for(let i = 1; i < limit && counter < limit; i++){
        ...
        const [anchor] = await page.$x('//*[@id="search-app"]/div/div[1]/div[2]/div[4]/div[1]/div/div['+(i+add)+']/div[1]/a');       
        if(anchor === undefined){
...
async function scrapeProductTrendyol(browser, productLink) {
    ...
    const product = {
        ...
        "storage": "SSD",
        "screen": await page.$x('//*[@id="product-detail-app"]/div/section/div/ul//span[text()="Ekran Boyutu"]/following-sibling::span/b').then((res) => page.evaluate(el => el.textContent, res[0])),
        "score": score,
...
async function scrapeProductTeknosa(browser, productLink) {
    ...
    const product = {
        ...
        "processor": await page.$x('//*[@id="pdp-technical"]/div/div[1]/div/table[count(//th[text()="İşlemci"]/parent::*/parent::*/parent::table/preceding-sibling::table)+1]//td[count(//th[text()="İşlemci"]/preceding-sibling::*)+1]').then((res) => page.evaluate(el => el.textContent, res[0])),
        ...
        "capacity": await page.$x('//*[@id="pdp-technical"]/div/div[1]/div/table[count(//th[contains(text(), "Kapasite")]/parent::*/parent::*/parent::table/preceding-sibling::table)+1]//td[count(//th[contains(text(), "Kapasite")]/preceding-sibling::*)+1]').then((res) => page.evaluate(el => el.textContent, res[0])),
        "storage": "SSD",
...
(async () => {
    ...
    const ListN11 = await scrapeListTrendyol(browser);
    //const ListVatan = await scrapeListVatan(browser);
    let productList = [];
    for(let i = 0; i < ListN11.length; i++){
        const product = await scrapeProductTrendyol(browser, ListN11[i]);
